# Диалог по подключению датчика протечки воды Gidrolock WSP через транзистор КТ853Б к плате WT32-ETH01 (с исправленной распиновкой v1.4)
...
## 🔌 Сборка Датчика Протечки воды Gidrolock WSP на транзисторе КТ853Б
...
**Ответ:** У датчика Gidrolock WSP полярности нет. При намокании он просто замыкает контакты. Вы можете поменять белый и коричневый провода местами, схема продолжит работать на 100% правильно. Но для порядка мы подключаем коричневый через 1 кОм к базе, а белый на GND.
...
4. К этой же **Базе (левая ножка 1)** подключи резистор **1 кОм**. Ко второму концу этого резистора подключи **коричневый провод** датчика Gidrolock WSP.
5. **Белый провод** датчика Gidrolock WSP подключи в синюю рельсу `[-]`.
