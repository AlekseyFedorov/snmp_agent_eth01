...
| 33 | Gidrolock WSP Протечка | INPUT_PULLUP | Через NPN-транзистор (2N3904) |
...
│   ├── sensors_app.c/h     # Драйверы датчиков (DS18B20, Gidrolock WSP, герконы)
...
Транзисторная развязка работает, но при пробое транзистора 5V попадает на GPIO 33. **Оптрон PC817** даёт гальваническую развязку — напряжение шлейфа (до 24V) физически изолировано от ESP32. Схема: WSP(+) → резистор 1кОм → анод оптрона → катод → WSP(-). Коллектор оптрона → GPIO 33, эмиттер → GND.
